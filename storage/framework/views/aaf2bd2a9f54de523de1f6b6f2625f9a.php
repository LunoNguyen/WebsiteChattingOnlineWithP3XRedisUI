...
<div class="flex-1 overflow-y-auto p-6 bg-gray-50">
  <div class="max-w-3xl mx-auto space-y-5">
    ...
    <div class="flex gap-2">
      ...
      <a href="<?php echo e(route('admin.backup')); ?>" class="bg-sky-500 text-white text-sm font-medium px-4 py-2 rounded-lg">💾 Backup</a>
    ...
    <?php $__currentLoopData = ['success','error','info']; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $type): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
      <?php if(session($type)): ?>
        <div class="bg-<?php echo e($type==='success'?'green':($type==='error'?'red':'sky')); ?>-50 border border-<?php echo e($type==='success'?'green':($type==='error'?'red':'sky')); ?>-200 text-<?php echo e($type==='success'?'green':($type==='error'?'red':'sky')); ?>-700 rounded-lg px-4 py-3 text-sm">
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-5">
      ...
      <form method="POST" action="<?php echo e(route('admin.backup.create')); ?>" class="flex gap-2">
        <?php echo csrf_field(); ?>
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
      <div class="px-5 py-3 border-b border-gray-100 flex items-center justify-between">
        ...
        <span class="text-xs text-gray-400"><?php echo e(count($backups)); ?> bản sao lưu</span>
      </div>
      ...
      <div class="px-5 py-3.5 border-b border-gray-50 last:border-0 flex items-center gap-3 hover:bg-gray-50/60 transition">
        <div class="w-10 h-10 rounded-xl flex items-center justify-center flex-shrink-0
          <?php echo e($backup['status'] === 'done' ? 'bg-emerald-100 text-emerald-600' : ($backup['status'] === 'failed' ? 'bg-red-100 text-red-600' : 'bg-yellow-100 text-yellow-600')); ?>">
          <?php if($backup['status'] === 'done'): ?>
        ...
        <div class="flex-1 min-w-0">
          <div class="flex items-center gap-2">
            <p class="text-sm font-semibold text-gray-900 truncate font-mono">
              <?php echo e($backup['file_name'] ?? ('backup_' . substr($backup['backup_id'], 0, 8) . '.json')); ?>

            </p>
            <?php if(!empty($backup['size_bytes'])): ?>
              <span class="text-[11px] px-1.5 py-0.5 rounded bg-gray-100 text-gray-600 font-mono">
                <?php echo e(number_format($backup['size_bytes'] / 1024, 1)); ?> KB
              </span>
            <?php endif; ?>
          </div>
          <p class="text-xs text-gray-500 truncate mt-0.5">
            <span class="text-gray-700 font-medium"><?php echo e($backup['note'] ?: 'Không có ghi chú'); ?></span> ·
            <?php echo e(\Carbon\Carbon::createFromTimestampMs($backup['created_at'])->format('d/m/Y H:i:s')); ?>
        ...
        <div class="flex items-center gap-2 flex-shrink-0">
          <?php if($backup['status'] === 'done'): ?>
            ...
            <a href="<?php echo e(route('admin.backup.download', $backup['backup_id'])); ?>"
            ...
            <form method="POST" action="<?php echo e(route('admin.backup.restore')); ?>"
              ...
              <?php echo csrf_field(); ?>
              <input type="hidden" name="backup_id" value="<?php echo e($backup['backup_id']); ?>">
              <button type="submit"
          ...
          <form method="POST" action="<?php echo e(route('admin.backup.destroy', $backup['backup_id'])); ?>"
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\laragon\www\chat-app\resources\views/admin/backup.blade.php ENDPATH**/ ?>