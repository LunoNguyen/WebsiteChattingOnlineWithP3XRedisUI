...
<head>
  ...
  <title><?php echo $__env->yieldContent('title', 'ChatApp'); ?> — ChatApp</title>
  ...
  <style>
    ...
    @media (max-width: 767px) {
      ...
      /* Trên điện thoại, luôn hiển thị đầy đủ mục và nút đăng xuất */
      ...
      #main-sidebar.collapsed .sidebar-header-logo {
        margin: 0 !important;
      }
      ...
      #main-sidebar.collapsed .user-info-box {
        justify-content: flex-start !important;
      }
      #main-sidebar.collapsed .logout-form {
...
<body class="bg-gray-100 h-screen flex overflow-hidden text-gray-900 relative">
  ...
  <aside id="main-sidebar" class="w-64 bg-gray-900 flex flex-col h-full flex-shrink-0 transition-all duration-300 ease-in-out relative border-r border-gray-800 select-none">
    ...
    <nav class="px-2 pt-3 space-y-1">
      <a href="<?php echo e(route('chat.index')); ?>" title="Tin nhắn"
        class="sidebar-link relative flex items-center gap-3 px-3 py-2.5 rounded-lg text-gray-300 hover:text-white transition <?php echo e(request()->routeIs('chat.*') ? 'active text-white' : ''); ?>">
        ...
        <span id="sidebar-chat-badge" class="sidebar-badge ml-auto bg-sky-500 text-white text-xs font-bold rounded-full px-2 py-0.5 <?php echo e(($totalUnread ?? 0) > 0 ? '' : 'hidden'); ?>">
          <?php echo e(($totalUnread ?? 0) > 99 ? '99+' : ($totalUnread ?? 0)); ?>

        </span>
      </a>

      <a href="<?php echo e(route('friends.index')); ?>" title="Bạn bè"
        class="sidebar-link relative flex items-center gap-3 px-3 py-2.5 rounded-lg text-gray-300 hover:text-white transition <?php echo e(request()->routeIs('friends.*') ? 'active text-white' : ''); ?>">
        ...
        <span id="sidebar-friend-badge" class="sidebar-badge ml-auto bg-amber-500 text-white text-xs font-bold rounded-full px-2 py-0.5 animate-pulse <?php echo e(($pendingFriendRequestsCount ?? 0) > 0 ? '' : 'hidden'); ?>" title="<?php echo e($pendingFriendRequestsCount ?? 0); ?> lời mời kết bạn mới">
          <?php echo e($pendingFriendRequestsCount ?? 0); ?>

        </span>
      </a>

      <a href="<?php echo e(route('groups.index')); ?>" title="Nhóm"
        class="sidebar-link relative flex items-center gap-3 px-3 py-2.5 rounded-lg text-gray-300 hover:text-white transition <?php echo e(request()->routeIs('groups.*') ? 'active text-white' : ''); ?>">
      ...
      <?php if(($authUser ?? null)?->isAdmin()): ?>
      <a href="<?php echo e(route('admin.users')); ?>" title="Quản trị"
        class="sidebar-link relative flex items-center gap-3 px-3 py-2.5 rounded-lg text-yellow-400 hover:text-yellow-200 transition <?php echo e(request()->routeIs('admin.*') ? 'active' : ''); ?>">
    ...
    <div class="p-2.5 border-t border-gray-800">
      <div class="user-info-box flex items-center gap-2">
        <a href="<?php echo e(route('profile')); ?>" class="flex items-center gap-2 flex-1 min-w-0 hover:bg-gray-800 rounded-lg p-1.5 transition" title="<?php echo e($authUser?->getName()); ?>">
          <div class="relative flex-shrink-0">
            <?php if(($authUser ?? null)?->avatar_url): ?>
            ...
            <?php else: ?>
              <div class="w-8 h-8 rounded-full bg-sky-500 flex items-center justify-center text-white text-sm font-bold">
                <?php echo e(strtoupper(substr($authUser?->getName() ?? 'U', 0, 1))); ?>
          ...
          <div class="sidebar-text min-w-0">
            <p class="text-white text-xs font-semibold truncate"><?php echo e($authUser?->getName()); ?></p>
          </div>
        </a>
        <form method="POST" action="<?php echo e(route('logout')); ?>" class="logout-form flex-shrink-0">
          <?php echo csrf_field(); ?>
      ...
      <div class="mt-2.5 pt-2 border-t border-gray-800 md:hidden">
        <form method="POST" action="<?php echo e(route('logout')); ?>" class="w-full">
          <?php echo csrf_field(); ?>
          <button type="submit" class="w-full flex items-center justify-center gap-2 py-2 px-3 rounded-lg bg-red-500/15 text-red-400 hover:bg-red-500/25 active:scale-98 font-medium text-xs transition border border-red-500/20 shadow-xs">
            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 16l4-4m0 0l-4-4m4 4H7m6 4v1a3 3 0 01-3 3H6a3 3 0 01-3-3V7a3 3 0 013-3h4a3 3 0 013 3v1"/></svg>
  ...
  <main class="flex-1 flex flex-col overflow-hidden">
    <?php if(($pendingFriendRequestsCount ?? 0) > 0 && !request()->routeIs('friends.*')): ?>
      <div class="bg-gradient-to-r from-amber-500 via-orange-500 to-amber-600 text-white px-4 py-2.5 text-xs font-medium flex items-center justify-between shadow-sm flex-shrink-0">
        <div class="flex items-center gap-2">
          ...
          <span>Bạn có <strong class="underline font-bold"><?php echo e($pendingFriendRequestsCount); ?> lời mời kết bạn mới</strong> đang chờ phản hồi!</span>
        </div>
        <a href="<?php echo e(route('friends.index')); ?>" class="bg-white text-amber-800 px-3 py-1 rounded-lg text-xs font-bold hover:bg-amber-50 transition shadow-xs flex items-center gap-1">
  ...
  <script>
    ...
    let globalLastUnread = <?php echo e((int)($totalUnread ?? 0)); ?>;
    let globalLastRequests = <?php echo e((int)($pendingFriendRequestsCount ?? 0)); ?>;
    let globalLastMsgAt = 0;

    async function pollGlobalStatus() {
      try {
        const res = await fetch('<?php echo e(route("chat.status.poll", [], false)); ?>', {
...
<?php /**PATH C:\laragon\www\chat-app\resources\views/layouts/app.blade.php ENDPATH**/ ?>