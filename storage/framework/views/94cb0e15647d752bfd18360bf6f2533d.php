...
<div class="flex h-full flex-col">
  ...
  <div class="bg-white border-b border-gray-200 px-4 py-3 flex items-center gap-3 flex-shrink-0">
    ...
    <div class="flex-1 min-w-0">
      <h3 class="font-semibold text-gray-900 text-sm flex items-center gap-2 truncate">
        <span><?php echo e($groupInfo['group']->name); ?></span>
      ...
      <p class="text-xs text-gray-400 truncate">
        <span id="header-member-count"><?php echo e(count($groupInfo['members'])); ?></span> thành viên
    ...
    <div class="bg-white rounded-2xl w-full max-w-xl shadow-2xl overflow-hidden flex flex-col max-h-[85vh]">
      ...
      <div class="px-5 py-4 border-b border-gray-100 flex items-center justify-between">
        <div class="flex items-center gap-2">
          <h4 class="font-bold text-gray-900 text-base">Quản lý nhóm "<?php echo e($groupInfo['group']->name); ?>"</h4>
        </div>
      ...
      <div class="flex border-b border-gray-200 px-5 gap-4 text-xs font-semibold bg-gray-50/50">
        <button type="button" onclick="switchManageTab('members')" id="mtab-btn-members"
          ...
          Thành viên (<?php echo e(count($groupInfo['members'])); ?>)
        </button>
      ...
      <div class="flex-1 overflow-y-auto p-5">
        ...
        <div id="mtab-content-members" class="space-y-2.5">
          <?php $__currentLoopData = $groupInfo['members']; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $m): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
            ...
            ?>
            <div class="flex items-center justify-between p-2.5 rounded-xl border border-gray-100 bg-gray-50/60 hover:bg-gray-50 transition" id="member-row-<?php echo e($m['user']->user_id); ?>">
              <div class="flex items-center gap-3 min-w-0">
                <?php if($m['user']->avatar_url): ?>
                  <img src="<?php echo e($m['user']->avatar_url); ?>" class="w-9 h-9 rounded-full object-cover flex-shrink-0">
                <?php else: ?>
                ...
                <div class="min-w-0">
                  <p class="text-xs font-semibold text-gray-900 truncate">
                    ...
                    <?php if($m['nickname']): ?>
                      <span class="text-[10px] text-gray-400 font-normal">(<?php echo e($m['user']->getName()); ?>)</span>
                    <?php endif; ?>
                    <?php if($isSelf): ?>
              ...
              <div class="flex items-center gap-2 flex-shrink-0">
                ...
                <span class="text-[10px] px-2 py-0.5 rounded-full font-semibold
                  <?php echo e($isTargetOwner ? 'bg-amber-100 text-amber-800' : ($isTargetAdmin ? 'bg-blue-100 text-blue-800' : 'bg-gray-100 text-gray-600')); ?>">
                  <?php echo e($isTargetOwner ? 'Trưởng nhóm' : ($isTargetAdmin ? 'Phó nhóm' : 'Thành viên')); ?>

                </span>

                
                <?php if($myRole === 'owner' && !$isSelf): ?>
                  
                  <?php if($isTargetAdmin): ?>
                    <button type="button" onclick="demoteMember('<?php echo e($m['user']->user_id); ?>', '<?php echo e($m['user']->getName()); ?>')"
                      class="text-[10px] text-blue-700 bg-blue-50 hover:bg-blue-100 px-2 py-1 rounded transition" title="Hạ cấp về thành viên">
                      Hạ cấp
                    </button>
                  <?php else: ?>
                    <button type="button" onclick="promoteMember('<?php echo e($m['user']->user_id); ?>', '<?php echo e($m['user']->getName()); ?>')"
                  ...
                  <button type="button" onclick="transferGroupOwnership('<?php echo e($m['user']->user_id); ?>', '<?php echo e($m['user']->getName()); ?>')"
                  ...
                  <button type="button" onclick="kickMember('<?php echo e($m['user']->user_id); ?>', '<?php echo e($m['user']->getName()); ?>')"
                    class="text-[10px] text-red-600 bg-red-50 hover:bg-red-100 px-2 py-1 rounded transition font-medium" title="Xóa khỏi nhóm">
                    Xóa
                  </button>
                <?php elseif($myRole === 'admin' && !$isTargetOwner && !$isTargetAdmin && !$isSelf): ?>
        ...
        <?php if(in_array($myRole, ['owner', 'admin'])): ?>
          <div id="mtab-content-info" class="hidden space-y-4">
            <form method="POST" action="<?php echo e(route('groups.update', $groupInfo['group']->group_id)); ?>" enctype="multipart/form-data" class="space-y-4">
                ...
                <div class="flex items-center gap-3.5 p-3 bg-gray-50 border border-gray-200 rounded-xl">
                  <div class="relative group cursor-pointer" onclick="document.getElementById('group-avatar-input').click()">
                    <div id="group-avatar-preview-box" class="w-14 h-14 rounded-full overflow-hidden bg-purple-600 flex items-center justify-center text-white font-bold text-lg shadow-xs border-2 border-white">
                      <?php if($groupInfo['group']->avatar_url): ?>
                        <img id="group-avatar-preview-img" src="<?php echo e($groupInfo['group']->avatar_url); ?>" class="w-full h-full object-cover">
                      <?php else: ?>
                        <span id="group-avatar-preview-text"><?php echo e(strtoupper(substr($groupInfo['group']->name, 0, 1))); ?></span>
                      <?php endif; ?>
                    </div>
              ...
              <div>
                ...
                <input type="text" name="name" value="<?php echo e($groupInfo['group']->name); ?>" required maxlength="100"
              ...
              <div>
                ...
                <textarea name="description" rows="2" maxlength="500"
                  ...
                  placeholder="Mô tả về nhóm..."><?php echo e($groupInfo['group']->description); ?></textarea>
              </div>

              <input type="hidden" name="avatar_url" value="<?php echo e($groupInfo['group']->avatar_url); ?>">
        ...
        <?php else: ?>
          ...
          <div id="mtab-content-leave" class="hidden space-y-4">
            <div class="bg-gray-50 border border-gray-200 rounded-xl p-4 text-center">
              ...
              <form method="POST" action="<?php echo e(route('groups.leave', $groupInfo['group']->group_id)); ?>">
                <?php echo csrf_field(); ?>
  ...
  <div id="nickname-modal" class="hidden fixed inset-0 bg-black/50 flex items-center justify-center z-50">
    <div class="bg-white rounded-xl p-5 w-80 shadow-xl">
      ...
      <form method="POST" action="<?php echo e(route('groups.nickname', $groupInfo['group']->group_id)); ?>">
        ...
        ?>
        <input type="hidden" name="target_user_id" value="<?php echo e($authUser->user_id); ?>">
  ...
  <div id="messages-container" class="flex-1 overflow-y-auto px-4 py-4 space-y-3 bg-gray-50">
    ...
    <?php $__currentLoopData = $messages; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $msg): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
      ...
      ?>
      <div class="flex <?php echo e($isMine ? 'justify-end' : 'justify-start'); ?> items-center gap-2 group msg-row py-1" id="msg-row-<?php echo e($msg->msg_id); ?>" data-msg-id="<?php echo e($msg->msg_id); ?>">
        ...
        <?php if($isMine && !$msg->isDeleted()): ?>
          <div class="actions-panel opacity-0 group-hover:opacity-100 transition-opacity flex items-center gap-1 bg-white border border-gray-200 rounded-lg shadow-sm px-1.5 py-1 self-center flex-shrink-0">
            <button onclick="replyToMsg('<?php echo e($msg->msg_id); ?>')" class="px-2 py-0.5 text-xs text-gray-600 hover:text-purple-600 hover:bg-purple-50 rounded font-medium transition">Trả lời</button>
            ...
            <button onclick="editMsg('<?php echo e($msg->msg_id); ?>')" class="px-2 py-0.5 text-xs text-gray-600 hover:text-purple-600 hover:bg-purple-50 rounded font-medium transition">Sửa</button>
        ...
        <?php if(!$isMine): ?>
          <div class="flex-shrink-0 self-end mb-1">
            <?php if($senderAvatarUrl): ?>
              <img src="<?php echo e($senderAvatarUrl); ?>" class="w-8 h-8 rounded-full object-cover">
            <?php else: ?>
              <div class="w-8 h-8 rounded-full bg-purple-600 flex items-center justify-center text-white text-xs font-bold">
                <?php echo e(strtoupper(substr($senderName, 0, 1))); ?>
        ...
        <div class="max-w-[70%]">
          ...
          <p class="text-xs font-semibold <?php echo e($isMine ? 'text-purple-600 text-right mr-1' : 'text-gray-700 ml-1'); ?> mb-1">
          ...
          <?php if($replyMsgRef): ?>
            ...
            ?>
            <div class="reply-preview-block mb-1 border-l-2 <?php echo e($isMine ? 'border-purple-400 bg-purple-50/70' : 'border-gray-400 bg-gray-100'); ?> px-2 py-1 rounded text-xs opacity-90 cursor-pointer hover:opacity-100 transition"
                 onclick="document.getElementById('msg-row-<?php echo e($replyMsgRef->msg_id); ?>')?.scrollIntoView({behavior:'smooth',block:'center'})">
                  ...
                  <?php $__currentLoopData = $pollData['options']; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $idx => $opt): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                    ...
                    <button type="button" onclick="castVote('<?php echo e($msg->msg_id); ?>', <?php echo e($idx); ?>)"
                      class="w-full text-left p-2 rounded-lg border transition relative overflow-hidden text-xs
                        <?php echo e($votedThis ? 'border-purple-300 bg-white/20 font-bold' : 'border-gray-200/40 hover:bg-black/5'); ?>

                        <?php echo e($isMine ? 'text-white' : 'text-gray-800 bg-gray-50'); ?>">
                      <div class="absolute inset-y-0 left-0 <?php echo e($votedThis ? 'bg-purple-400/40' : 'bg-gray-300/30'); ?>" style="width: <?php echo e($pct); ?>%"></div>
                ...
                <div class="w-64 p-2.5 rounded-xl border <?php echo e($isMine ? 'bg-white/10 border-white/20 text-white' : 'bg-gray-50 border-gray-200 text-gray-800'); ?> shadow-xs">
                  <div class="flex items-start gap-2.5">
                    <div class="w-10 h-10 rounded-lg flex items-center justify-center font-bold text-xs flex-shrink-0 shadow-xs
                      <?php echo e(in_array($ext, ['doc','docx']) ? 'bg-blue-600 text-white' : (in_array($ext, ['pdf']) ? 'bg-red-500 text-white' : (in_array($ext, ['xls','xlsx']) ? 'bg-emerald-600 text-white' : 'bg-purple-600 text-white'))); ?>">
                  ...
                  <div class="flex items-center gap-2 mt-2.5 pt-2 border-t <?php echo e($isMine ? 'border-white/15' : 'border-gray-200'); ?>">
                    <button type="button" onclick="previewFile('<?php echo e($fUrl); ?>', '<?php echo e(addslashes($fName)); ?>', '<?php echo e($fSize); ?>', '<?php echo e($ext); ?>')"
                      class="flex-1 py-1 px-2 rounded-lg text-xs font-medium text-center transition flex items-center justify-center gap-1
                        <?php echo e($isMine ? 'bg-white/20 hover:bg-white/30 text-white' : 'bg-purple-50 hover:bg-purple-100 text-purple-700'); ?>">
        ...
        <?php if($isMine): ?>
          <div class="flex-shrink-0 self-end mb-1">
            <?php if($authUser->avatar_url): ?>
              <img src="<?php echo e($authUser->avatar_url); ?>" class="w-8 h-8 rounded-full object-cover">
            <?php else: ?>
              <div class="w-8 h-8 rounded-full bg-indigo-500 flex items-center justify-center text-white text-xs font-bold">
                <?php echo e(strtoupper(substr($authUser->getName(), 0, 1))); ?>
...
const GROUP_ID     = '<?php echo e($groupInfo["group"]->group_id); ?>';
...
const PIN_STORE_KEY = 'pin_group_<?php echo e($groupInfo["group"]->group_id); ?>';
(function() {
