...
<div class="flex h-full flex-col">
  ...
  <div class="bg-white border-b border-gray-200 px-4 py-3 flex items-center gap-3 flex-shrink-0">
    ...
    <div class="flex-1">
      <h3 class="font-semibold text-gray-900 text-sm">
        ...
        @if($nickname)<span class="text-gray-400 font-normal text-xs ml-1">({{ $otherUser->username }})</span>@endif
      </h3>
      <p class="text-xs {{ $otherUser->is_online ? 'text-green-500' : 'text-gray-400' }}">
    ...
    <div class="flex items-center gap-2">
      @if(!$isBlocked)
        ...
        <button onclick="document.getElementById('nickname-modal').classList.toggle('hidden')"
          class="p-2 text-gray-400 hover:text-sky-500 hover:bg-gray-100 rounded-lg transition" title="Đặt nickname">
      ...
      <form method="POST" action="{{ $isBlocked ? route('friends.unblock') : route('friends.block') }}">
        ...
        <button type="submit" class="p-2 text-gray-400 hover:text-red-500 hover:bg-gray-100 rounded-lg transition"
          title="{{ $isBlocked ? 'Bỏ chặn' : 'Chặn người dùng' }}">
  ...
  <div id="nickname-modal" class="hidden fixed inset-0 bg-black/50 flex items-center justify-center z-50">
    <div class="bg-white rounded-xl p-5 w-80 shadow-xl">
      <h4 class="font-semibold text-gray-900 mb-3">Đặt nickname cho {{ $otherUser->getName() }}</h4>
      <form method="POST" action="{{ route('friends.nickname') }}">
        ...
        <input type="text" name="nickname" value="{{ $nickname }}"
          ...
          placeholder="Nhập nickname (để trống để xóa)">
  ...
  <div id="messages-container" class="flex-1 overflow-y-auto px-4 py-4 space-y-3 bg-gray-50">
    @if($isBlocked)
      <div class="text-center text-gray-400 text-sm py-8">Bạn đã chặn người dùng này.</div>
    @else
      @forelse($messages as $msg)
        @php $isMine = $msg->sender_id === $authUser->user_id; @endphp
        <div class="flex {{ $isMine ? 'justify-end' : 'justify-start' }} group" data-msg-id="{{ $msg->msg_id }}">
          <div class="max-w-[70%]">
            ...
            <div class="relative px-4 py-2 text-sm shadow-sm
              {{ $isMine
                ? 'bg-sky-500 text-white chat-bubble-mine'
              ...
              @if($isMine && !$msg->isDeleted())
                <div class="absolute -top-7 right-0 hidden group-hover:flex items-center gap-1 bg-white border border-gray-100 rounded-lg shadow px-1 py-0.5">
                  <button onclick="editMsg('{{ $msg->msg_id }}', {{ json_encode($msg->content) }})"
                    class="p-1 text-gray-500 hover:text-sky-500 text-xs">✏️</button>
  ...
  <div class="bg-white border-t border-gray-200 px-4 py-3 flex-shrink-0">
    ...
    <form id="send-form" class="flex items-end gap-2">
      ...
      <textarea id="message-input" rows="1"
        ...
        placeholder="Nhập tin nhắn..." maxlength="5000"
      ...
      <button type="button" onclick="sendOrEdit()"
        class="bg-sky-500 hover:bg-sky-600 text-white rounded-xl p-2.5 transition flex-shrink-0">
...
const SEND_URL  = '{{ route("chat.dm.send", $otherUser->user_id) }}';
