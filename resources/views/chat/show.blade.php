@extends('layouts.app')
@section('title', $nickname ?: $otherUser->getName())
...
<div class="flex h-full flex-col">
  ...
  <div class="bg-white border-b border-gray-200 px-4 py-3 flex items-center gap-3 flex-shrink-0">
    <a href="{{ route('chat.index') }}" class="p-1.5 text-gray-400 hover:text-gray-600 rounded-lg hover:bg-gray-100 transition mr-1" title="Quay lại danh sách chat">
    ...
    <div class="relative">
      ...
      @else
        <div class="w-10 h-10 rounded-full bg-sky-500 flex items-center justify-center text-white font-bold">
          {{ strtoupper(substr($otherUser->getName(), 0, 1)) }}
        </div>
      @endif
    ...
    <div class="flex-1 min-w-0">
      ...
      <p class="text-xs {{ $otherUser->is_online ? 'text-green-500 font-medium' : 'text-gray-400' }}">
        {{ $otherUser->is_online ? 'Đang online' : 'Offline' }}
      </p>
    </div>
    ...
    <div class="flex items-center gap-1.5">
      ...
      <form method="POST" action="{{ $isBlocked ? route('friends.unblock') : route('friends.block') }}"
        onsubmit="return confirm('{{ $isBlocked ? 'Gỡ chặn cho người này?' : 'Chặn người dùng này? Hai người sẽ không thể nhắn tin cho nhau.' }}')">
        @csrf
        <input type="hidden" name="target_id" value="{{ $otherUser->user_id }}">
        <button type="submit" class="p-2 {{ $isBlocked ? 'text-red-500 bg-red-50 hover:bg-red-100' : 'text-gray-400 hover:text-red-500 hover:bg-gray-100' }} rounded-lg transition"
          title="{{ $isBlocked ? 'Bấm để gỡ chặn' : 'Chặn người dùng' }}">
  ...
  @if($isBlocked)
    <div class="bg-red-50 border-b border-red-200 px-4 py-3 flex items-center justify-between">
      ...
      <form method="POST" action="{{ route('friends.unblock') }}" onsubmit="return confirm('Gỡ chặn người này?')">
        @csrf
        <input type="hidden" name="target_id" value="{{ $otherUser->user_id }}">
  ...
  <div id="nickname-modal" class="hidden fixed inset-0 bg-black/50 flex items-center justify-center z-50">
    <div class="bg-white rounded-xl p-5 w-80 shadow-xl">
      <h4 class="font-semibold text-gray-900 mb-3 text-sm">Đặt biệt danh cho {{ $otherUser->getName() }}</h4>
      <form method="POST" action="{{ route('friends.nickname') }}">
        @csrf
        <input type="hidden" name="friend_id" value="{{ $otherUser->user_id }}">
  ...
  <div id="messages-container" class="flex-1 overflow-y-auto px-4 py-4 space-y-3 bg-gray-50">
    ...
    @foreach($messages as $msg)
      ...
      @if($isSystem)
        {{-- Tin nhắn hệ thống (thông báo bình chọn,...) căn giữa --}}
      ...
      <div class="flex {{ $isMine ? 'justify-end' : 'justify-start' }} items-center gap-2 group msg-row py-1"
           id="msg-row-{{ $msg->msg_id }}" data-msg-id="{{ $msg->msg_id }}">
        ...
        @if($isMine && !$msg->isDeleted())
          <div class="actions-panel opacity-0 group-hover:opacity-100 transition-opacity flex items-center gap-1 bg-white border border-gray-200 rounded-lg shadow-sm px-1.5 py-1 self-center flex-shrink-0">
            ...
            <button onclick="editMsg('{{ $msg->msg_id }}')" class="px-2 py-0.5 text-xs text-gray-600 hover:text-sky-600 hover:bg-sky-50 rounded font-medium transition">Sửa</button>
        ...
        @if(!$isMine)
          <div class="flex-shrink-0 self-end mb-1">
            @if($otherAvatar)
              <img src="{{ $otherAvatar }}" class="w-8 h-8 rounded-full object-cover">
            @else
        ...
        <div class="max-w-[70%]">
          {{-- Sender Name --}}
          <p class="text-xs font-semibold {{ $isMine ? 'text-sky-600 text-right mr-1' : 'text-gray-700 ml-1' }} mb-1">
            {{ $senderDisplayName }}
          </p>
          ...
          @if($replyMsg)
            ...
            <div class="reply-preview-block mb-1 border-l-2 {{ $isMine ? 'border-sky-400 bg-sky-50/70' : 'border-gray-400 bg-gray-100' }} px-2 py-1 rounded cursor-pointer text-xs transition hover:opacity-90"
                 onclick="document.getElementById('msg-row-{{ $replyMsg->msg_id }}')?.scrollIntoView({behavior:'smooth',block:'center'})">
                ...
                <div class="w-64 p-2.5 rounded-xl border {{ $isMine ? 'bg-white/10 border-white/20 text-white' : 'bg-gray-50 border-gray-200 text-gray-800' }} shadow-xs">
                  <div class="flex items-start gap-2.5">
                    <div class="w-10 h-10 rounded-lg flex items-center justify-center font-bold text-xs flex-shrink-0 shadow-xs
                      {{ in_array($ext, ['doc','docx']) ? 'bg-blue-600 text-white' : (in_array($ext, ['pdf']) ? 'bg-red-500 text-white' : (in_array($ext, ['xls','xlsx']) ? 'bg-emerald-600 text-white' : 'bg-gray-600 text-white')) }}">
                      {{ strtoupper($ext ?: 'FILE') }}
                    </div>
                    <div class="flex-1 min-w-0">
        ...
        @if($isMine)
          <div class="flex-shrink-0 self-end mb-1">
            @if($myAvatar)
              <img src="{{ $myAvatar }}" class="w-8 h-8 rounded-full object-cover">
            @else
...
const SEND_URL   = '{{ route("chat.dm.send", $otherUser->user_id, false) }}';
const FILE_URL   = '{{ route("chat.dm.file", $otherUser->user_id, false) }}';
const POLL_URL   = '{{ route("chat.dm.new", $otherUser->user_id, false) }}';
...
@php $convKeyForPin = 'pin_conv_' . ($conv->conv_id ?? ''); @endphp
const PIN_STORE_KEY = '{{ $convKeyForPin }}';
(function() {
